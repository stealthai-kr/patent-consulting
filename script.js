...
function showEmployeeBalanceSummary(store, name, phone) {
  ...
  renderEmployeeBalanceSummary(employee);
}
...
async function renderEmployeeBalanceSummary(selectedEmployee) {
  ...
  try {
    ...
    const leave = leaveResult.balance || {};
    ...
    const specialRemain = firstNumber(
      leave.specialRemain,
      leave.specialBalance,
      leave.specialLeaveRemain,
      special.remain,
      special.balance,
      Math.max(0, specialGranted - specialUsed - specialPending)
    );
    ...
    const spouseRemain = firstNumber(
      leave.spouseRemain,
      leave.spouseBalance,
      leave.spouseLeaveRemain,
      spouse.remain,
      spouse.balance,
      Math.max(0, spouseGranted - spouseUsed - spousePending)
    );

    const cardStyle = "padding:18px;border:1px solid #dce3eb;border-radius:16px;background:#fff;line-height:1.75;";
    const remainStyle = "margin-top:14px;padding-top:12px;border-top:1px solid #e7ebf0;font-size:20px;color:#178b59;font-weight:900;";

    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap;margin-bottom:18px;">
        <div>
          <div style="font-size:21px;font-weight:900;">${escapeHtml(name)} 휴가·미휴무 현황</div>
          <div style="color:#6f7785;margin-top:4px;">${escapeHtml(store)} · ${escapeHtml(formatEmployeePhone(phone))}</div>
        </div>
        <button type="button" class="btn btn-secondary btn-small" onclick="closeEmployeeBalanceSummary()">닫기</button>
      </div>

      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:14px;">
        <div style="${cardStyle}">
          <div style="font-size:17px;font-weight:900;color:#2f5f9e;margin-bottom:10px;">📅 연월차</div>
          발생 <strong>${firstNumber(leave.base)}일</strong><br>
          승인 사용 <strong>${firstNumber(leave.used)}일</strong><br>
          승인 대기 <strong>${firstNumber(leave.pending)}일</strong><br>
          이월 잔여 <strong>${firstNumber(leave.carryoverRemain)}일</strong>
          <div style="${remainStyle}">잔여 ${firstNumber(leave.remain)}일</div>
        </div>

        <div style="${cardStyle}">
          <div style="font-size:17px;font-weight:900;color:#9a6416;margin-bottom:10px;">🎁 특별휴가</div>
          부여 <strong>${specialGranted}일</strong><br>
          승인 사용 <strong>${specialUsed}일</strong><br>
          승인 대기 <strong>${specialPending}일</strong>
          <div style="${remainStyle}">잔여 ${specialRemain}일</div>
        </div>

        <div style="${cardStyle}">
          <div style="font-size:17px;font-weight:900;color:#7b4a9e;margin-bottom:10px;">👶 배우자 출산휴가</div>
          발생 <strong>${spouseGranted}일</strong><br>
          승인 사용 <strong>${spouseUsed}일</strong><br>
          승인 대기 <strong>${spousePending}일</strong>
          <div style="${remainStyle}">잔여 ${spouseRemain}일</div>
        </div>

        <div style="${cardStyle}">
          <div style="font-size:17px;font-weight:900;color:#c16f1d;margin-bottom:10px;">🕒 미휴무</div>
          승인 발생 <strong>${firstNumber(compResult.earned)}일</strong><br>
          승인 사용 <strong>${firstNumber(compResult.used)}일</strong><br>
          발생 대기 <strong>${firstNumber(compResult.pendingEarned)}일</strong><br>
          사용 대기 <strong>${firstNumber(compResult.pendingUsed)}일</strong>
          <div style="${remainStyle}">잔여 ${firstNumber(compResult.balance)}일</div>
        </div>
      </div>
    `;
...
/* 기존 loadEmployees 완료 뒤 직원 선택목록도 갱신 */
const _originalRenderEmployeesForLeaveGrant = renderEmployees;
renderEmployees = function() {
  const result = _originalRenderEmployeesForLeaveGrant.apply(this, arguments);
